...
import { Lobster, Poppins, Bebas_Neue } from "next/font/google";
import Link from "next/link";
import Image from "next/image";
import MedLogo from "../assets/images/medical-symbol-of-snake-caduceus-pink-26174_1024.png";
...
const Navbar = () => {
  const [isOpen, setIsOpen] = useState(false); // Controls the Monster Modal
  const [navbarOpen, setNavbarOpen] = useState(false); // Controls Mobile Dropdown
  ...
  const getLinkClass = (path: string) => {
    ...
    return `${bebas_neue.className} ${
      isActive ? "text-pink-500 font-bold" : "text-gray-400"
    } hover:text-pink-600 cursor-pointer transition list-none text-lg`;
  };

  // Smooth dropdown variants for mobile
  const menuVariants = {
    closed: {
      opacity: 0,
      height: 0,
  ...
  return (
    <div className="w-full shadow relative bg-zinc-950 border-b border-zinc-900 z-50">
      <nav className="w-full">
        {/* Changed from lg: to md: to show full menu on 13" MacBook scales */}
        <div className="justify-between px-4 mx-auto md:max-w-7xl md:items-center md:flex md:px-8">
          
          {/* Logo Area & Mobile Trigger */}
          <div>
            <div className="flex items-center justify-between py-3 md:py-5 md:block">
              ...
              <div className="md:hidden">
                <button
                  className="p-2 text-gray-400 rounded-md outline-none border border-transparent focus:border-zinc-700"
          ...
          {/* Desktop Links - Visible on 13-inch Macbook and Tablets */}
          <div className="hidden md:flex md:flex-1 md:justify-center">
            <ul className="items-center justify-center space-x-6 flex">
              <li><Link href="/" className={getLinkClass("/")}>Home</Link></li>
              <li><Link href="/facts" className={getLinkClass("/facts")}>Facts</Link></li>
          ...
          {/* Desktop View Button - Visible on 13-inch Macbook and up */}
          <div className="hidden md:inline-block">
            <button 
              onClick={toggleModal}
              className="bg-green-500 hover:bg-green-400 text-black font-black py-1.5 px-3 rounded-md cursor-pointer transition active:scale-95 flex items-center gap-2 shadow-lg shadow-green-500/10 group"
            >
              <svg 
                className="w-5 h-5 object-contain transform group-hover:scale-110 transition-transform duration-200" 
      ...
      {/* Animated Mobile Dropdown Menu (Only targets screens below md: 768px) */}
      <AnimatePresence initial={false}>
        {navbarOpen && (
          <motion.div
            ...
            animate="open"
            exit="closed"
            className="overflow-hidden md:hidden w-full absolute top-full left-0 bg-zinc-950/95 backdrop-blur-md border-b border-zinc-900 shadow-xl"
          >
            <ul className="flex flex-col items-center space-y-4 pb-6 pt-4 px-4">
              <li><Link href="/" onClick={() => setNavbarOpen(false)} className={getLinkClass("/")}>Home</Link></li>
              <li><Link href="/facts" onClick={() => setNavbarOpen(false)} className={getLinkClass("/facts")}>Facts</Link></li>
              ...
              <li className="w-full max-w-xs pt-2">
                <button 
                  onClick={() => { toggleModal(); setNavbarOpen(false); }}
                  className="bg-green-500 hover:bg-green-400 text-black font-black py-2 px-3 rounded-md cursor-pointer transition active:scale-95 flex items-center gap-2 shadow-lg shadow-green-500/10 justify-center w-full"
                >
                  <span className={`tracking-tight text-white ${poppins.className} text-sm font-bold`}>Get a Monster Here!</span>
                </button>
              </li>
            </ul>
          </motion.div>
      ...
      {isOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in">
          <div className="bg-zinc-900 border-2 border-red-500 rounded-2xl p-8 max-w-sm w-full text-center shadow-2xl shadow-red-500/20">
            <h1 className="text-3xl font-black text-red-500 tracking-wider mb-2">
              ❌ CRITICAL ERROR
            </h1>
            <p className="text-xl font-bold text-white mb-4">
              BAD FOR HEALTH
            </p>
            <p className="text-sm text-zinc-400 font-serif italic mb-6 border-l-2 border-zinc-700 pl-3 text-left">
              "Do you really want another lecture from Christine?"
            </p>
            
            <button 
              onClick={toggleModal} 
              className="w-full bg-red-600 hover:bg-red-500 text-white font-mono font-bold py-2 px-4 rounded-xl transition duration-200"
            >
              Close (Accept Defeat)
            </button>
          </div>
